...
With the updated syntax from the author, I replicated the direction of all the mixed model tests. Many of the major numerical errors came from this test - it's hard to tell whether they are due to differences in SPSS version or are actually major numerical reporting errors.

Note that the model initially fit in `R` (model `m1`) used the default exchangeable correlation structure, yet still does not agree closely with published results. 
