...
Unclear what test they used - probably a t-test?
```{r}
t.test(AVO ~ gender, data=dataset1, paired=FALSE)
t.test(ANX ~ gender, data=dataset1, paired=FALSE)
```

If using t-test, the lowest value reported doesn't match reported. 
```{r}
compareValues(reportedValue = 0.11, obtainedValue = 0.0931, isP=T)
```

This discrepency is larger if repeated scores for each subject are taken into account
```{r}
...
First, calculate congruency effect. 
```{r}
#START HERE - SHOULD BE WORKING BUT IT IS NOT
dataset2.long$congruency = as.factor(dataset2.long$congruency)
spread(dataset2.long, congruency, mean_latency, convert=T, drop=T)
