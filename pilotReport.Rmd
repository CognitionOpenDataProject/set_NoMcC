...
However, I was not able to reproduce the mixed linear model described in the results section. Even within statistical programs, different procedures calculate these models different, so some variance is to be expected. However, I did not see the same pattern of significant results (specifically in the interaction between relationship and ANX attachment scores), even when I attempted to reproduce the results in SPSS using the same GENLINMIXED procedure. It's worth noting that I ran the results on SPSS 23, and the authors used SPSS 21, which could have implemented a different procedure for this model. 
