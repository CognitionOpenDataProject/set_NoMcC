...
The reported df are not the same as I see (30 reported versus 12 observed). It almost seems like the model didn't actually use ID as a random effect and just used a normal OLS regression. 
```{r}
m2 = lm(CE ~ relationship*ZANX*ZAVO, data=dataset3)
summary(m2)
anova(m2)
```
