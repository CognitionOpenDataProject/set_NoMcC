...
Unclear what test they used - probably a t-test, perhaps separate t-tests for each test/relationship since they said "all" p-values rather than "both"?
```{r}
...
Although if done in an ANOVA (which is what I had tried first), it's not the same result. But if this is about reproducing what they did, it passes. 
```{r}
...
Running the RM anova and calculating table. 
```{r}
...
Run linear mixed effects model. START HERE
```{r}
...
```{r, run=FALSE}
write.csv(dataset3, file="TestforSPSS.csv", row.names = FALSE)
```
...
* Read in data file. 
GET DATA  /TYPE=TXT
  /FILE="\\tsclient\Desktop\TestforSPSS.csv"
  /DELCASE=LINE
  /DELIMITERS=","
  /QUALIFIER='"'
